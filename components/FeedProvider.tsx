...
export function FeedProvider({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (next.fingerprint !== feedRef.current.fingerprint || next.fetchedAt !== feedRef.current.fetchedAt) apply(next);
    } catch {
      failuresRef.current += 1;
      setRefreshError("Couldn’t check for updates. Showing the last available snapshot.");
    } finally {
  ...
  const value = useMemo(
    ...
    [feed, notice, refreshing, checkedAt, refreshError, refresh, newsHref, patchHref],
  );
